...
# hidden layer: 
h_weight = tf.Variable(tf.random_normal([n_inputs, n_hidden_units]))
h_bias = tf.Variable(tf.constant(0.1, shape=[n_hidden_units, ]))
input_x_reshape = tf.reshape(input_x, [-1, n_inputs]) # (256 batch, 28 steps, 28) => (256 batch * 28, 28)
y1 = tf.matmul(input_x_reshape, h_weight) + h_bias # => (256 batch * 28, 128)
...
o_bias = tf.Variable(tf.constant(0.1, shape=[n_classes, ]))
output_rnn_ = tf.unstack(tf.transpose(output_rnn, perm=[1, 0, 2])) # transpose to (steps(time) * batch * data)
prediction = tf.matmul(output_rnn_[-1], o_weight) + o_bias # use last output from RNN
# accuracy:
