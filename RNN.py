...
# (256 batch, 28 steps, 28) => (256 batch * 28, 28)
input_x_reshape = tf.reshape(input_x, [-1, n_inputs])
y1 = tf.matmul(input_x_reshape, encoder_weight) + \
    encoder_bias  # => (256 batch * 28, 128)
# => (256 batch, 28 steps, 128)
y1_reshape = tf.reshape(y1, [-1, n_steps, n_hidden_units])
# RNN:
# cell = tf.contrib.rnn.BasicRNNCell(n_hidden_units) # basic RNN
# cell = tf.contrib.rnn.BasicLSTMCell(n_hidden_units) # LSTM
# cell = tf.contrib.rnn.LSTMCell(n_hidden_units) # LSTM *able to set
# peephole, clip
cell = tf.contrib.rnn.GRUCell(n_hidden_units)  # GRU
# state = cell.zero_state(batch_size, dtype=tf.float32)
# time_major=False for data format = (batch, time, data)
output_rnn, final_state = tf.nn.dynamic_rnn(
    cell, y1_reshape, time_major=False, dtype=tf.float32)
...
# transpose to (steps(time) * batch * data)
output_rnn_ = tf.unstack(tf.transpose(output_rnn, perm=[1, 0, 2]))
# use last output from RNN
prediction = tf.matmul(output_rnn_[-1], decoder_weight) + decoder_bias
# accuracy:
accuracy = tf.reduce_mean(
    tf.cast(tf.equal(tf.argmax(prediction, 1), tf.argmax(output_y, 1)), tf.float32))
# loss:
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
    logits=prediction, labels=output_y))
# optimizer: Adam
