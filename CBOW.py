...
def generate_batch(batch_size, scan_num, scan_window):
    ...
    input
        ...
        scan_num: how many words need to collect for each word -> batch_size / scan_num = how many word in each batch
        scan_window: limit collect range = target word +- scan_window
    output
        batch: related word with target (batch_size * scan_num)
...
# weights(50,000*128) X inputs(128*128) + biases(50,000*1) => 50,000*128 = result for each trainning sample
    ...
    # => output[0] = 50,000*1 = the propability for target 
